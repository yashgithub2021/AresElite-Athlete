...
const Card4 = () => {
 ...
 const [selectedActivity,setSelectedActivity]=useState([])
  ...
  return (
    <>
     <Modal.Root opened={modal2} onClose={()=>{modaloptions.close()}}  size="50vw">
        <Modal.Overlay />
        <Modal.Content>
          <Modal.Header>
            <Modal.Title><h4>{selectedActivity.length>0 && selectedActivity[0][0]?.activityName}</h4></Modal.Title>
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={()=>{modaloptions.close()}}>
<path d="M0 19.2C0 12.4794 0 9.11905 1.30792 6.55211C2.4584 4.29417 4.29417 2.4584 6.55211 1.30792C9.11905 0 12.4794 0 19.2 0H20.8C27.5206 0 30.8809 0 33.4479 1.30792C35.7058 2.4584 37.5416 4.29417 38.6921 6.55211C40 9.11905 40 12.4794 40 19.2V20.8C40 27.5206 40 30.8809 38.6921 33.4479C37.5416 35.7058 35.7058 37.5416 33.4479 38.6921C30.8809 40 27.5206 40 20.8 40H19.2C12.4794 40 9.11905 40 6.55211 38.6921C4.29417 37.5416 2.4584 35.7058 1.30792 33.4479C0 30.8809 0 27.5206 0 20.8V19.2Z" fill="#F4F4F4"/>
<path d="M16.4654 15.5805C16.2213 15.3364 15.8256 15.3364 15.5815 15.5805C15.3374 15.8246 15.3374 16.2203 15.5815 16.4644L19.1171 20L15.5815 23.5355C15.3375 23.7795 15.3375 24.1753 15.5815 24.4194C15.8256 24.6634 16.2213 24.6634 16.4654 24.4194L20.0009 20.8838L23.5364 24.4194C23.7805 24.6634 24.1763 24.6634 24.4203 24.4194C24.6644 24.1753 24.6644 23.7795 24.4203 23.5355L20.8848 20L24.4204 16.4644C24.6645 16.2203 24.6645 15.8246 24.4204 15.5805C24.1763 15.3364 23.7806 15.3364 23.5365 15.5805L20.0009 19.1161L16.4654 15.5805Z" fill="#1C1C1C"/>
</svg>

          </Modal.Header>
          <Modal.Body>
          <div className="form-grid" style={{padding:"30px"}}>
           {selectedActivity.length>0 && selectedActivity[0][0]?.form.map((item)=>{
            return(
              <>
...
<path d="M16.4654 15.5805C16.2213 15.3364 15.8256 15.3364 15.5815 15.5805C15.3374 15.8246 15.3374 16.2203 15.5815 16.4644L19.1171 20L15.5815 23.5355C15.3375 23.7795 15.3375 24.1753 15.5815 24.4194C15.8256 24.6634 16.2213 24.6634 16.4654 24.4194L20.0009 20.8838L23.5364 24.4194C23.7805 24.6634 24.1763 24.6634 24.4203 24.4194C24.6644 24.1753 24.6644 23.7795 24.4203 23.5355L20.8848 20L24.4204 16.4644C24.6645 16.2203 24.6645 15.8246 24.4204 15.5805C24.1763 15.3364 23.7806 15.3364 23.5365 15.5805L20.0009 19.1161L16.4654 15.5805Z" fill="#1C1C1C"/>
          ...
          <div className="weekbuttons" style={{width:"100%" ,overFlowX:"scroll"}}>
          ...
          <div style={{padding:"30px"}}>
            <h5 style={{marginBottom:"10px"}}>Day index <hr/></h5>
            {drill_week_details?.drills.map((item)=>{
              return item.activities.map((activity,index)=>{
               ...
               return(
                ...
                <div>
                <h7 style={{color:"#3C3F53CC"}}>{activity.activityName}</h7>
           <p style={{color:"#3C3F53CC"}}>{activity.description}</p>
                </div>
                <div>
                 <button className="week-button week-button-selected" style={{fontSize:"small"}} onClick={()=>{secondModalHandler();filterformodal(index)}} >View Drill</button>
                </div>
           </div>
               )
              })
  ...
  {isFetching?<></>:<></>}
    <div
      xs={6}
      sm={6}
      className={isFetching?`upper-card text-shadow pulsate`:`upper-card text-shadow`}
      style={{
        background: "#7257FF",
        width: "420px",
      }}
        ...
        <div>
            ...
            {[...Array(totalWeeks)]?.map((item, index) => {
              return (
                <button
                  ...
                  style={{ minWidth: "64px", padding: " 4px 8px 4px 8px" }}
                >
          ...
          <div>
            ...
            >
              ...
              <button style={{ color: "inherit" }} onClick={open}>View All</button>
            </div>
            <hr />
          </div>
          <div>
               ...
               >
                 <div style={{ color: "inherit" }}>
                   <div style={{textAlign:"left"}}>
                     <p style={{ fontSize: "18px", margin: "0px" }}>{activ.activityName}</p>
                     <p
                       style={{
                         fontSize: "12px",
                         margin: "0px",
                 ...
                 <button style={{ color: "#7257FF",background:"white",padding:"7.5px 10px 7.5px 10px",borderRadius:"8px"}} onClick={()=>{secondModalHandler();filterformodal(index)}}>View Drill</button>
               </div>
               )
            })
